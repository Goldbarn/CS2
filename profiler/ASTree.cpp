...
AST::~AST() {
    ...
    std::list<AST*>::iterator deletee = child.begin();
    while (!child.empty() && deletee != child.end()) {
...
AST::AST(const AST& actual) {
    ...
    // std::list<AST*> childrenActual = actual.child;
    // const std::list<AST*>::iterator it = childrenActual.begin();
    // while (it != child.end()) {
    //     AST temp(it);
    //     child.push_back(new AST(it));
    //     childrenActual.pop_front();
    //     ++it;
    // }
...
void AST::mainHeader(const std::vector<std::string>& profileName,
        ...
        while ((*it)->tag != "function") { ++it; }

        child.insert(it, new AST(token, "#include \"profile.hpp\"\n"));
        std::vector<std::string>::const_iterator itP = profileName.begin(); 
        std::vector<std::string>::const_iterator itF = fileName.begin();

        while (itP != profileName.end() && itF != fileName.end()) {

            child.insert(it, new AST(token, "profile " + *itP + "(\"" + *itF + "\");\n"));
            ++itP; ++itF;
        }
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    while (it != child.end()) {
        ...
        if ((*it)->tag == "function" && ((*it)->getChild("name"))->getName() == "main") {
            ...
            std::list<AST*>::iterator itFin = ((*it)->getChild("block"))->getChild("block_content")->child.end();
            --itFin;
            --itFin;

            //while (((*itFin)->getChild("return") == "return") { --itFin; }

            std::vector<std::string>::const_iterator PN = profileName.begin();
            while (PN != profileName.end()) {

                (*it)->child.insert(itFin, new AST(token, "std::cout << " + *PN + " << std::endl;\n"));
                ++PN;
...
void AST::functionCount(const std::string& profileName) {
    ...
    do {
        ...
        if ((*it)->tag == "function" || (*it)->tag == "constructor" || (*it)->tag == "destructor") { 
            ...
            std::string funcName = (*it)->getChild("name")->getName();
            std::list<AST*>::iterator itFin = ((*it)->getChild("block"))->getChild("block_content")->child.begin();
            (*itFin)->child.insert(itFin, new AST(token, profileName + ".count(__LINE__, \"" + funcName + "\");"));
...
void AST::lineCount(const std::string& profileName) {
    ...
    for (std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i) { std::cerr << "d";

        if ((*i)->nodeType == category) {

            if ((*i)->tag == "expr_stmt") {

                (*i)->child.insert(++i, new AST(token, profileName + ".count(__LINE__);"));
                --i;
